...
export class InterproceduralPass implements AnalysisPass<InterproceduralPassResult> {
  ...
  run(ctx: PassContext): InterproceduralPassResult {
    ...
    if (sinks.length === 0) {
      ...
      // Low-confidence `interprocedural_param` sources (confidence < 0.6) arise
      // when the IR extractor fails to read the parameter type (e.g. TypeScript
      // constructor params with `private`/`public` modifiers), causing the
      // type-unaware fallback to mark all params as potentially tainted.
      // Excluding both prevents false-positive `external_taint_escape` sinks
      // whenever library utility methods pass their own constructor arguments.
      const fallbackSources = sources.filter(
        s => s.type !== 'constructor_field' &&
          !(s.type === 'interprocedural_param' && s.confidence < 0.6),
      );
